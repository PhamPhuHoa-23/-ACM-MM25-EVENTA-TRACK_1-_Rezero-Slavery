...
3. **Memory issues with large datasets:**
   ...
   - Use `--skip_upload` for existing collections

4. **Image not found errors:**
   - Verify image paths in `--query_images_path` and `--database_images_path`
   - Ensure image IDs match between vectors and actual images

### Performance Tips

- Use SSD storage for better I/O performance
- Increase Qdrant memory allocation for large collections
- Run re-ranking stages incrementally for debugging
- Use `--skip_stage*` flags to resume from specific stages
